...
def compute_bases_overlapped(feature_file, bp_file, input=None):
    if bp_file == 'stdin':
        ...
        random_bp_hits = subprocess.Popen(['intersectBed', '-a', feature_file, '-b', bp_file, '-wo', '-sorted'], stdout=subprocess.PIPE).communicate()[0]
    bases_overlapped = 0
...
if process_iteration_cmd == None:
    ...
    if not replot:
        ...
        for i in xrange(-1000000,1000000,25000):
            ...
            bp_hits = float(compute_bp_hits(feature_file, 'stdin', "\n".join(filtered_shifted_regions))) / len(filtered_shifted_regions)
            shifted_bp_hits_file.write(str(i) + "\t" + str(bp_hits) + "\n")
            feature_hits = float(compute_feature_hits(feature_file, 'stdin', "\n".join(filtered_shifted_regions))) / len(filtered_shifted_regions)
            shifted_feature_hits_file.write(str(i) + "\t" + str(feature_hits) + "\n")
            bases_overlapped = float(compute_bases_overlapped(feature_file, 'stdin', "\n".join(filtered_shifted_regions))) / len(filtered_shifted_regions)
