...
SAMPE_CMD="/g/whelanch/software/bin/bwa sampe -n 25 -N 25 -r '@RG\tID:"${RGID}"\tLB:"${LIBRARY}"\tSM:"${SAMPLE}"' $REFERENCE $READ1_BASE.sai $READ2_BASE.sai $READ_FILE1 $READ_FILE2 | samtools view -Sb - > $OUTFILE_PREFIX.bam"
echo $SAMPE_CMD
eval $SAMPE_CMD
